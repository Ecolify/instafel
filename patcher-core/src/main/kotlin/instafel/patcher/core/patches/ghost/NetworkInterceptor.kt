    ...
    override fun initializeTasks() = mutableListOf(
        ...
        object : InstafelTask() {
            override fun execute() {
                // TigonServiceLayer is in a standard package structure, not obfuscated
                // Search using getSmaliFilesByName instead of X-folder search
                ...
                if (candidates.size > 1) {
                    // Multiple matches - should be very rare, but filter by exact class declaration
                    ...
                    if (exactMatches.size == 1) {
                        tigonServiceLayerFile = exactMatches[0]
                        success("TigonServiceLayer class found after filtering: ${tigonServiceLayerFile.absolutePath}")
                    } else if (exactMatches.isEmpty()) {
                        failure("Patch aborted: No exact TigonServiceLayer class found among ${candidates.size} candidates")
                        return
                    } else {
                        tigonServiceLayerFile = exactMatches.first()
                        success("TigonServiceLayer class selected (first of ${exactMatches.size}): ${tigonServiceLayerFile.absolutePath}")
                    }
                } else {
                    // Single match - verify it has startRequest method
